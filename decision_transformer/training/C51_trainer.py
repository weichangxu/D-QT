    ...
    def train_step(self, log_writer=None, loss_metric={}):
        ...
        if self.k_rewards:
            if self.max_q_backup:
                ...
                target_q1_dist, target_q2_dist = self.critic_target(critic_next_states, next_action)
                target_q1_dist = target_q1_dist.view(batch_size, repeat_num, -1).max(dim=1)[0]
                target_q2_dist = target_q2_dist.view(batch_size, repeat_num, -1).max(dim=1)[0]
            else:
                critic_next_states = states[:, -1]
                next_action = next_action[:, -1]
                target_q1_dist, target_q2_dist = self.critic_target(critic_next_states, next_action) # [B, 51]

            # 计算最小 Q 值的分布
            target_q_dist = self.critic.min(target_q1_dist, target_q2_dist) # [B, 51]
            ...
            if self.use_discount:
                ...
                k_rewards = k_rewards / discount # [B, T, 1]

                discount = [i - 1 - torch.arange(i) for i in mask_]  # [B]
                discount = torch.stack([torch.cat([torch.zeros(T - len(i)), i], dim=0) for i in discount], dim=0) # [B, T]
                discount = (self.discount ** discount).to(device)  # [B, T]

                # **投影 Q 值分布**
                target_q_dist = self.critic.compute_target_distribution(k_rewards, dones, target_q_dist, self.discount)
                target_q_mean = (target_q_dist * self.critic.support.to(target_q_dist.device)).sum(dim=-1)  # 计算 Q 值均值
            ...
            if self.max_q_backup:
                target_q1_dist, target_q2_dist = self.critic_target(states_rpt, next_action)  # [B*repeat, T, 51]
        
                # 取最大 Q 值的概率分布（Max Q Backup）
                target_q1_dist = target_q1_dist.view(batch_size, repeat_num, T, -1).max(dim=1)[0]
                target_q2_dist = target_q2_dist.view(batch_size, repeat_num, T, -1).max(dim=1)[0]
        ...
        # 计算 KL 散度损失
        loss_q1 = F.kl_div(current_q1_dist.log(), target_q_dist, reduction='batchmean')
        loss_q2 = F.kl_div(current_q2_dist.log(), target_q_dist, reduction='batchmean')
