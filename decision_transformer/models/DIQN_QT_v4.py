...
class IQN(nn.Module):
    def __init__(self, state_size, action_size, layer_size, seed=123, N=32, dueling=False, device="cuda:0"):
        ...
        # Define the second Q value network (for double Q learning)
        self.head_2 = nn.Linear(self.action_size+self.input_shape, layer_size) 
        self.cos_embedding_2 = nn.Linear(self.n_cos, layer_size)
        self.ff_1_2 = nn.Linear(layer_size, layer_size)
        self.ff_2_2 = nn.Linear(layer_size, 1)  
    ...
    def forward(self, input, action, num_tau=32):
        ...
        # Multiply the cosine values with the input and propagate through the network
        x = (x.unsqueeze(1)*cos_x).view(Batch_Size * num_tau, self.layer_size)
        x = torch.relu(self.ff_1(x))

        out_1 = self.ff_2(x)

        # Second Q network (for Double Q Learning)
        x_2 = torch.cat((input, action), dim=1)
        x_2 = torch.relu(self.head_2(x_2))
        
        cos_2, taus_2 = self.calc_cos(Batch_Size, num_tau)  # cos shape (batch, num_tau, layer_size)
        cos_2 = cos_2.view(Batch_Size * num_tau, self.n_cos)
        cos_x_2 = torch.relu(self.cos_embedding_2(cos_2)).view(Batch_Size, num_tau, self.layer_size)  # (batch, n_tau, layer)
        
        # Multiply the cosine values with the input and propagate through the second network
        x_2 = (x_2.unsqueeze(1)*cos_x_2).view(Batch_Size * num_tau, self.layer_size)
        x_2 = torch.relu(self.ff_1_2(x_2))

        out_2 = self.ff_2_2(x_2)
        
        # Return the minimum of the two Q-values (double Q-learning)
        return out_1.view(Batch_Size, num_tau, 1), out_2.view(Batch_Size, num_tau, 1), taus
    
    def get_qvalues(self, inputs, action):
        quantiles_1, quantiles_2,  _ = self.forward(inputs, action, self.N)
        actions_1 = quantiles_1.mean(dim=1)
        actions_2 = quantiles_2.mean(dim=1)

        return actions_1,  actions_2
    
    def q_min(self, inputs, action):
        quantiles_1, quantiles_2,  _ = self.forward(inputs, action, self.N)
        actions_1 = quantiles_1.mean(dim=1)
        actions_2 = quantiles_2.mean(dim=1)
        return torch.min(actions_1,  actions_2)
